module.exports = function (router) {
  ...
  var version = '0-1-5'
  var base_url = 'raa-v2/' + version + '/sprint/ER-324-trading';
  ...
  router.get('/' + base_url + '*', function (req, res,next) {
    ...
    if(req.query.TradingName){req.session.TradingName=req.query.TradingName}
    ...
    req.session.numberOfPositions =  req.session.numberOfPositions = 1;
  ...
  router.post('/' + base_url + '/create-vacancy-options/display-employer', function (req, res) {
    ...
    if(req.body.selected_display_name== "anonymous"){
      ...
      if(req.body.continueButton){
        ...
        res.redirect(301, '/' + base_url + '/create-vacancy-options/location-anonymous?edit=yes&updatedLegalEntity='+req.query.updatedLegalEntity)
      }
    }else{
      ...
      if(req.body.continueButton){
        ...
        res.redirect(301, '/' + base_url + '/create-vacancy-options/location?edit=yes&updatedLegalEntity='+req.query.updatedLegalEntity)
      }
    }
  })
  router.get('/' + base_url + '/create-vacancy-options/display-employer-tradingname', function (req, res) {

    req.session.hasTradingName = "yes";
    res.render(base_url + '/create-vacancy-options/display-employer-tradingname', {
      "data" : req.session,
      "query" : req.query,
      }
    )
  })
  router.post('/' + base_url + '/create-vacancy-options/display-employer-tradingName', function (req, res) {

    if(req.body.selected_display_name == "tradingName"){
    req.session.hasTradingName = "yes"
    }else{
      req.session.hasTradingName = "no"
      req.session.TradingName = req.body.TradingName
    }
  ...
  router.post('/' + base_url + '/create-vacancy-options/employer', function (req, res) {
    ...
    var updatedLegalEntity = (req.session.orgName == req.body.selected_organisation_id) ? "no" : "yes";
    console.log(updatedLegalEntity)
    if(updatedLegalEntity == "yes"){
        req.session.hasTradingName = "no"
    }
    ...
    if(req.body.continueButton){
      ...
      if(req.session.hasTradingName == "yes"){
        res.redirect(301, '/' + base_url + '/create-vacancy-options/display-employer-tradingName?edit=yes&updatedLegalEntity='+updatedLegalEntity)
      }else{
        res.redirect(301, '/' + base_url + '/create-vacancy-options/display-employer?edit=yes&updatedLegalEntity='+updatedLegalEntity)
      }
    }
  ...
  router.post('/' + base_url + '/create-vacancy-options/location-anonymous', function (req, res) {
    if(req.body.continueButton){
      res.redirect(301, '/' + base_url + '/create-vacancy-options/training')
    }else{
      res.redirect(301, '/' + base_url + '/create-vacancy-options/training?edit=yes')
    }
