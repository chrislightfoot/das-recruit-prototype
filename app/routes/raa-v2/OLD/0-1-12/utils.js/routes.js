module.exports = function(router) {
  ...
  router.post('/' + base_url + '*/create/select-training-provider', function(req, res) {
    ...
    if (req.body.has_training_provider == "yes") {
      ...
      res.redirect(301, '/' + base_url + req.params[0] + "/create/confirm-training-provider?providerName=" + name + "&UKPRN=" + ukprn)
  ...
  router.get('/' + base_url + '*', function(req, res) {
    ...
    console.log("backup file == " + baseDir)
    res.render(base_url + req.params[0], function(err, html) {
      if (err) {
